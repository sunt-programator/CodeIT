...
**CodeIT** 主题使用内容摘要在主页中显示大致文章信息。Hugo 支持生成文章的摘要.
...
你仍然需要在文章开头添加 `<!--more-->` 摘要分割符. 将摘要分隔符之前的内容保留为空. 然后 **CodeIT** 主题会将你的文章描述作为摘要.
...
**CodeIT** 基于 [$ \KaTeX $](https://katex.org/) 提供数学公式的支持.
...
**CodeIT** 主题支持一种 **字符注音或者注释** Markdown 扩展语法:
...
**CodeIT** 主题使用 [Font Awesome](https://fontawesome.com/) 作为图标库.
你同样可以在文章中轻松使用这些图标.
